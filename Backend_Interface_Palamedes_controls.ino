...
MultiStepper steppersControl;  // Create instance of MultiStepper to move them at the same time
Servo myservo; //create instance of servo
int pos1=0;//variables for servo
int pos=0;
long gotoposition[2];
long gotoparking[2]={8000,9000};
long zSafety=[500];
long zValue;
float lowSpeed;
float highSpeed;
    ...
    while (failure){
      if(Serial.available()>0){
        ...
        if(orderCheck=="reset"){
          ...
          steppersControl.moveTo(gotoparking); 
          stepperX.setMaxSpeed(highSpeed);
          stepperY.setMaxSpeed(highSpeed);
  ...
  if (commandFlag && order != previousOrder && failure==false){
    ...
    if (order.substring(0,1)=="J"){
      digitalWrite(bluePin,HIGH);
  	  ...
  	  gotoposition[0]=control(joint1);
  	  gotoposition[1]=control(joint2);
...
void findStartPosition(){
  ...
  stepperX.moveTo(gotoparking[0]);//positive
  ...
  stepperY.moveTo(gotoparking[1]);//positif
  stepperY.runToPosition();
  //for Z
  while (digitalRead(Z_MIN_PIN) != 1) {
    stepperZ.setSpeed(500);//positif
    stepperZ.runSpeed();
  }
  ...
  stepperZ.moveTo(-2000);//negatif //le seul qui va pas à un point particulier pour le moment
...
void reverseKinematics(float X, float Y){
  ...
  choice=false; 
  gotoposition[0]= steps1+8000;
  gotoposition[1]= -steps2+7466;
